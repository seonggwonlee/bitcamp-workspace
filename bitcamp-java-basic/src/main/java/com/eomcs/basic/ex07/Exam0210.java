...
public class Exam0210 {
  ...
  public static void main(String[] arg) {
    ...
    m3(); // => 반드시 리턴 값을 받을 필요는 없다.

    // int result = m3(); => String이기 때문에 int 불가능
    // message = m3(100); => 불가능
  ...
  static void m8(String name, int...scores) { // 가변 파라미터 (메서드 안에서는 배열로 다룬다.)
    int sum = 0;
    for (int i = 0; i < scores.length; i++) {
      sum += scores[i];
    }
...
//  static void m9(int...scroes, String name) {
//  }

//  가변 파라미터는 여러 개 선언할 수 없다.
//  static void m9(int...scroes, int...scores2) {
//  }

//  가변 파라미터 중간에 다른 타입의 변수가 오더라도
//  가변 파라미터를 한 개를 초과하여 둘 수 없다.
