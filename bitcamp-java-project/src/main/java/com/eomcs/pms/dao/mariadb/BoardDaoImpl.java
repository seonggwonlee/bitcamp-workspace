...
public class BoardDaoImpl implements com.eomcs.pms.dao.BoardDao{
  ...
  public int insert(Board board) throws Exception {
    ...
    // mybatis는 기본 수동 커밋으로 동작한다.
    // 즉 sqlSession.openSession()은 수동 커밋이다. => sqlSession.openSession(true)가 자동 커밋
    try (SqlSession sqlSession = sqlSessionFactory.openSession(true)) {

      // SqlSession 객체에 insertSQL문을 실행하라고 명령한다.
  ...
  public List<Board> findAll() throws Exception {
    ...
    // SqlSession객체는 SQL문을 실행하는 객체다.
    try (SqlSession sqlSession = sqlSessionFactory.openSession()) {

      // SqlSession객체를 통해 별도 파일에 분리한 SQL을 찾아 실행하라고 명령한다.
